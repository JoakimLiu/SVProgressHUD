...
知识点：
 1、自定义layer:SVRadialGradientLayer，这个开源项目也是HUD(https://github.com/Jinxiansen/JHUD/) 里面的样式也是用layer来处理的 可以了解一下
...
/// 整个层级 overlayView -> self -> hudView -> imageView or ringView or indefiniteAnimatedView and statusLabel
...
- (void)updateHUDFrame {
    ...
    if (string) {
        ...
        if ([string respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
            stringRect = [string boundingRectWithSize:constraintSize options:(NSStringDrawingOptions)(NSStringDrawingUsesFontLeading | NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin) attributes:@{ NSFontAttributeName: self.statusLabel.font } context:NULL];
        } else {
            CGSize stringSize;
...
#endif
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(positionHUD:) name:UIApplicationDidBecomeActiveNotification object:nil];
}
...
- (void)overlayViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent *)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:SVProgressHUDDidReceiveTouchEventNotification object:self userInfo:[self notificationUserInfo]];

    UITouch *touch = event.allTouches.anyObject;
    CGPoint touchLocation = [touch locationInView:self];

    if (CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:SVProgressHUDDidTouchDownInsideNotification object:self userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showStatus:(NSString *)status {
    ...
    if (self.alpha != 1.0f || self.hudView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:SVProgressHUDWillAppearNotification object:self userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if (strongSelf) {
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:SVProgressHUDDidAppearNotification object:strongSelf userInfo:[strongSelf notificationUserInfo]];
            }
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if (strongSelf) {
            ...
            if (strongSelf.alpha != 0.0f || strongSelf.hudView.alpha != 0.0f) {
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:SVProgressHUDWillDisappearNotification object:nil userInfo:[strongSelf notificationUserInfo]];
                ...
                __block void (^completionBlock)(void) = ^{
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:SVProgressHUDDidDisappearNotification object:strongSelf userInfo:[strongSelf notificationUserInfo]];
